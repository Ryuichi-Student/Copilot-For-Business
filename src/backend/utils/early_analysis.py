...
def early_analysis(userQuery, db: Database):
    system_prompt = dedent('''\
        ...
        If the status is "schema", it indicates that the query can be resolved by interpreting the schema itself. Please include in the message a detailed and insightful explanation that directly answers the user's query, leveraging the schema information. 
            ...
            - If the user asks questions like "What questions can I ask about this database?" or "What are the possible queries that can be answered by this database?", the status should be "schema" and message should include an analysis, along with list of possible queries that can be answered by the database through executing SQL commands; Query answers are allowed to be a format that is suitable for graph generation (Bar chart, Line Chart, Pie Chart, Scatter Chart), or just plain text and dataframe. 
